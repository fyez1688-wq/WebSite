...
管理员可填写合法音频 URL，也可上传自己有权使用的 MP3、M4A、OGG、WAV。上传文件会校验 MIME、文件头和大小，交由当前 StorageProvider 保存；当前仍不提供转码能力，以控制版权、存储和带宽风险。
...
- 支持合法音频 URL 和管理员受控上传的 MP3、M4A、OGG、WAV；暂不支持转码。
...
为什么仍要限制音频上传格式和大小？音频文件会带来存储、版权、转码和带宽风险。因此当前只允许管理员上传少量受控格式，校验文件内容与默认 50MB 上限，并且不提供转码或未知来源的批量导入。
