...
- 音乐播放器 UI：导航音乐面板和底部 MiniPlayer 已改为低高度、图标优先的横向控制布局；保留原有播放、暂停、上一首、下一首、进度和桌面音量逻辑。收藏心形仅作禁用视觉预留，后续如需收藏必须复用现有收藏模型或另行设计，不在本次实现。
...
- 标签文章草稿：第一阶段已对 94 个当前 Tag 各生成 1 篇 `ARTICLE` / `DRAFT`，共创建 94 篇，未发布。后续应在 `/admin/content` 按标签抽查标题、摘要、正文准确性、分类和 Markdown 排版，完成人工修订与审核后再逐篇发布。再次运行脚本前先执行 `npm run generate:tag-articles:dry -- --per-tag=1`，确认稳定 slug 的跳过统计；不得未经重新规划而调高每标签生成数量。
