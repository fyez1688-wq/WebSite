...
- 首页 Hero 宽度：顶部轮播/Hero 外层已统一复用全站 `1220px` 容器宽度与移动端边距，不应再将 Hero 恢复为桌面全宽背景。Hero 图片继续使用 `PublicImage`，远程 R2 图片保持直连策略。
- 音乐对象生命周期：后台删除歌曲已同步处理本站受控 `audio/<UUID>.<ext>`。仅当前 Provider 公开基址匹配的音频才可清理；外部链接、`covers/` 和其他前缀必须保留。同 URL 的其他未删除引用存在时保留对象，清理失败时不软删歌曲；后续若增加恢复/永久删除流程，需先明确音频文件是否仍可恢复，不能绕过这些边界。
