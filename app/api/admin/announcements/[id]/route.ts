...
export async function PATCH(request: Request, { params }: Params) {
  ...
  await prisma.operationLog.create({
    data: { actorId: session.user.id, action: "UPDATE", target: "Announcement", targetId: id, ip: await clientIp() }
  });
  return ok({ item }, "公告已更新");
}
...
export async function DELETE(_request: Request, { params }: Params) {
  ...
  await prisma.operationLog.create({
    data: { actorId: session.user.id, action: "DELETE", target: "Announcement", targetId: id, ip: await clientIp() }
  });
  return ok({ id }, "公告已删除");
}
